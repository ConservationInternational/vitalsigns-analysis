...
agvalue <- agvalue %>% group_by(Household.ID) %>% summarize(Crops = sum(ag4a_16, na.rm=T) - sum(ag4a_21, na.rm=T))
...
hv2.1 <- merge(hv2.1, hv2.1 %>% group_by(Country) %>% summarize(bundle_price=mean(hh_hv105b_01, na.rm=T)), all.x=T)
...
fw_plot <- hv2.1 %>% filter(Country=='TZA') %>% merge(landscape_df) %>% group_by(Location) %>%
  summarize(Fuelwood=sum(bundle_value, na.rm=T))

ggplot(fw_plot) + geom_bar(aes(x=Location, y=Fuelwood), stat='identity') + theme_bw()
  
  
hv2.1 <- hv2.1 %>% group_by(Country, Landscape..) %>% summarize(NR_Value=sum(all_nr, na.rm=T))
...
df <- merge(hv2.1, income)

df_m <- melt(df %>% filter(Country=='UGA') %>% select(Landscape.., Nature.Value=NR_Value, Ag.Income=income_net), id.vars='Landscape..')

ggplot(df_m) + geom_bar(aes(x=Landscape.., y=value, fill=variable), stat='identity', position='dodge')
