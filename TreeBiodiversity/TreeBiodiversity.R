...
eplot <- tbl(con, 'flagging__eplot') %>% select(survey_uuid, latitude, longitude, `Subplot Radius`, flag) %>% data.frame %>%
...
species_sum <- species %>% group_by(survey_uuid, Landscape.., Eplot.., Country, latitude, longitude) %>% 
...
IDW <- gstat::idw(biodiversity~1, locations=ss, newdata=grd, idp=1.0)
plot(IDW)
...
plot(cp, main='Biodiversity in Southern Tanzania', labels=F, xaxt='n', yaxt='n',
